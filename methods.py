    ...
    for name, path in modules.items():
        try:
            with open(os.path.join(path, "register_types.h")):
                includes_cpp += f'#include "{path}' + '/register_types.h"\n'
...
def save_active_platforms(apnames, ap):
    ...
    for x in ap:
        svg_names = []
        if os.path.isfile(f"{x}/logo.svg"):
            svg_names.append("logo")
        if os.path.isfile(f"{x}/run_icon.svg"):
            svg_names.append("run_icon")

        for name in svg_names:
            with open(f"{x}/{name}.svg", "rb") as svgf:
                b = svgf.read(1)
                svg_str = " /* AUTOGENERATED FILE, DO NOT EDIT */ \n"
                svg_str += f' static const char *_{x[9:]}_{name}_svg = "'
                while len(b) == 1:
                    svg_str += "\\" + hex(ord(b))[1:]
                    b = svgf.read(1)
...
def generate_vs_project(env, num_jobs, project_name="godot"):
    if batch_file := find_visual_c_batch_file(env):
        ...
        env.MSVSProject(
            target=[f"#{project_name}" + env["MSVSPROJECTSUFFIX"]],
            incs=env.vs_incs,
            srcs=env.vs_srcs,
